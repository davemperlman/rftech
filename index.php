<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
	<title>Refined Technologies | Website Services</title>
	<link rel="stylesheet" type="text/css" href="http://yui.yahooapis.com/3.18.1/build/cssreset/cssreset-min.css">
	<link rel="stylesheet" href="css/main.css">
	<link href="https://fonts.googleapis.com/css?family=Cabin:300,400,500,700" rel="stylesheet">
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
</head>
<body>
<!-- <div id="loading">
	<div id="loading-img">
		
	</div>
</div> -->
<div class="modal-wrap">
	<div class="modal">
		<h3>Build your web presence!</h3>
		<ul class="info">
			<li>Refined Technologies</li>
			<li><a href="">207-440-4576</a></li>
			<li><a href="">info@refined-tech.com</a></li>
		</ul>
		<div id="loading-img">
		
		</div>
		<form action="">
			<label for="email">email</label>
			<input type="email" name="email" id="email">
			<label for="subject">Subject</label>
			<input type="subject" name="subject" id="subject">
			<label for="message">message</label>
			<textarea name="message" id="message"></textarea>
			<span id="send">Send</span>
		</form>
		<p id='status'></p>
		<span id="close-modal">Close</span>
	</div>
</div>
<section>
	<header>
		<div class="logo">
			<img src="img/draft-text-white.png" alt="">
			<img src="img/draft-text-black.png" alt="">
		</div>
		<div id="nav-icon">
			<span></span>
			<span></span>
			<span></span>
		</div>
		<nav>
			<ul class="topnav" aria-label="Navigation">
				<li><a href="index.php">Home</a></li>
				<li><a href="#">About</a></li>
				<li>
					<span>Services</span>
					<ul class="hidden-services">
						<li class="hidden-service"><a href="responsivedesign.php">Responsive Design</a></li>
						<li class="hidden-service"><a href="databasedesign.php">Database Design</a></li>
						<li class="hidden-service"><a>Content Management</a></li>
						<li class="hidden-service"><a>SEO Marketing</a></li>
					</ul>
				</li>
				<li><a href="#">Contact</a></li>
			</ul>
		</nav>
	</header>
</section>
<section class="block banner">
	<!-- Mission Statement -->
	<div>
		<h1>Build a Strong <strong>Online Presence</strong></h1>
		<h2>with Refined Technologies' <strong>Web Solutions.</strong></h2>
		<!-- <p>Here is a short description about what we do!</p> -->
	<!-- 	<span class="modal-activate">Get Started</span> -->
		<img src="img/screen.png" alt="Web Solutions from your Computer.">
	</div>
	<span class="modal-activate">Get Started</span>
</section>
<div class="info-wrap">
	<section class="block about">
		<h3>Professional Web Development</h3>
		<p>Refined Technologies is a company dedicated to providing excellence in Web Development and Design. Lorem ipsum dolor sit amet, consectetur adipisicing elit. Possimus saepe nemo amet labore ad, natus placeat commodi aperiam voluptatibus. Neque in cupiditate repellendus illum necessitatibus odio culpa iusto eum animi! </p>
	</section>
	<section class="block info">
		<div>
			<h3>Online Marketing</h3>
			<img src="img/marketing.png" alt="">
			<p>Conceptualizing an internet presence can be an arduous task. What do I want to say - who do I want to say it to - how do I want to say it - These are all questions that we help you to answer. A site isn't just a site anymore. What your site offers will define you or your company in the eyes of your users.</p>
			<h3>Brand Design</h3>
			<img src="img/design.png" alt="">
			<p>The competitive market demands a business be fluid and ready to change. Utlizing the latest development and design trends keeps your web presence fresh and boasts a refreshing user experience for your clients.</p>
		</div>
	</section>
</div>
<div class="services-wrap">
	<section class="block services">
		<h3><strong>Website Services</strong></h3>
		<ul>
			<li>
				<img src="img/design2.png" alt="Responsive Design">
				<h4>Responsive Design</h4>
				<p>Provide your users with a sleek, modern visual experience that allows them to easily navigate. It's a creative process, no two designs are the same. We build our designs based on what you want the user experience to be.</p>				
			</li>
			<li>
				<img src="img/data.png" alt="Database Design">
				<h4>Database Design</h4>
				<p>The processing and manipulation of data is an often overlooked aspect of modern web applications. Contracts, email lists, customer databases, and the like are all dynamic information that can often be more efficiently managed by a web application. We can help you increase the interactivity of your site by building custom data management and processing functionality.</p>				
			</li>
			<li>
				<img src="img/wordpress.png" alt="Wordpress Site">
				<h4>Content Management</h4>
				<p>Managing content. As an administrator of a site, you should be able to easily modify and update content on your site. Add a blog post, upload media, change the title of an article, etc. These are all things that can be done without having to contact a developer. Keep more of your time.</p>
			</li>
			<li>
				<img src="img/seo.png" alt="Online Marketing">
				<h4>SEO Marketing</h4>
				<p>So you have a site, great! Now let's help people find it. There are BILLIONS of websites on the internet, we make sure that people are finding yours using proper semantics and targeted keywords.</p>	
			</li>
		</ul>
	</section>
	<!-- <section class="block services specs">
		<div id="design">
			<h4>Design</h4>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Saepe consequatur quasi, odio ab neque delectus excepturi. Tenetur enim ea accusantium commodi adipisci voluptatibus, tempora. Quos, magni optio tempore excepturi eveniet?</p>
		</div>
		<div id="data">
			<h4>Data Management</h4>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ratione, nisi, est laboriosam quas provident in minus maxime ad maiores rerum, sunt dolor culpa libero ex voluptates. Eos, temporibus recusandae natus!</p>
		</div>
		<div id="cms">
			<h4>Content Management</h4>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Labore vitae nulla, totam ab eligendi ratione eius laborum impedit est quo, saepe molestias mollitia nemo magni at? Unde, iste vel ullam.</p>
		</div>
		<div id="seo">
			<h4>Search Engine Optimization</h4>
			<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Assumenda accusantium aut nihil iure, laboriosam dolorem quasi numquam quisquam quibusdam odio repellat quia rem, illo alias sapiente, recusandae necessitatibus sint minus.</p>
		</div>
	</section> -->
</div>
<section class="foot">
	<footer>
		<span>Start a Project</span>
		<nav>
		<ul class="bottomnav" aria-label="Navigation">
			<li><a href="#">Home</a></li>
			<li><a href="#">About</a></li>
			<li><a href="#">Contact</a></li>
		</ul>
	</nav>
	<ul class="icons">
		<li>Facebook</li>
		<li>Twitter</li>
		<li>Instagram</li>
		<li>Social</li>
	</ul>
		<h2>
			<img src="img/notext.png" alt="">
		</h2>
		<section>
			Refined Technologies 2017
		</section>
	</footer>
</section>
<script src='js/menu.js'></script>
<script src='js/modal.js'></script>

</body>
</html>